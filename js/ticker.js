...
let ticker_update = () => {
  ...
  let current_prices = ticker_keys.map(
    element => data_obj[element][currency].slice(-1)[0].toFixed(2)
  );
  let strings = []
...
let get_history = () => {
  let dates = time_frames.forEach(element => moment().subtract(1,element).format("D-M-Y"))
}
